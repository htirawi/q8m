...
class DataGuard {
  ...
  checkDataFiles() {
    ...
    const allowedExtensions = ['.json', '.ts', '.js', '.md', '.csv'];

    for (const pattern of forbiddenPatterns) {
      try {
        // Find files matching pattern, excluding fixtures, node_modules, dist
        const result = execSync(
          `find . -path "./fixtures" -prune -o -path "./node_modules" -prune -o -path "./dist" -prune -o -path "./coverage" -prune -o -name "*${pattern.replace('**/', '')}*" -type f -print`,
        ...
        for (const file of files) {
          ...
          const hasForbiddenExt = allowedExtensions.some(ext => file.endsWith(ext));
  ...
  checkDataExports() {
    const dataPatterns = [
      { pattern: /export.*questions\s*=/i, message: 'Questions export detected' },
      { pattern: /export.*quiz\s*=/i, message: 'Quiz export detected' },
      { pattern: /const.*questions\s*=/i, message: 'Questions constant detected' },
      { pattern: /const.*quiz\s*=/i, message: 'Quiz constant detected' }
    ];

    try {
      // Search for data patterns in source files (excluding fixtures)
      const result = execSync(
        'find . -path "./fixtures" -prune -o -path "./node_modules" -prune -o -path "./dist" -prune -o -name "*.ts" -o -name "*.js" -o -name "*.vue" | grep -v fixtures | grep -v node_modules | grep -v dist',
        { encoding: 'utf8', cwd: process.cwd() }
      );
