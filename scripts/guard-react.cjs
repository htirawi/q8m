...
class ReactGuard {
  ...
  checkSourceCode() {
    const reactPatterns = [
      { pattern: /import\s+.*\s+from\s+['"]react['"]/i, message: 'React import detected' },
      { pattern: /import\s+React\s+from/i, message: 'React import detected' },
      { pattern: /React\./, message: 'React object usage detected' },
      { pattern: /\buseState\b|\buseEffect\b|\buseMemo\b|\buseCallback\b/i, message: 'React hooks detected' },
      { pattern: /\bcreateContext\b/i, message: 'React context detected' },
      { pattern: /\bStrictMode\b/i, message: 'React StrictMode detected' },
      { pattern: /\bReactDOM\b/i, message: 'ReactDOM usage detected' }
    ];

    try {
      // Search for React patterns in source files
      const result = execSync('find . -name "*.ts" -o -name "*.js" -o -name "*.vue" | grep -v node_modules | grep -v fixtures | grep -v dist | grep -v backups', { 
        encoding: 'utf8',
        cwd: process.cwd()
      });
