...
test.describe('Plan Unification - Conversion Flow', () => {
  test('should complete homepage → pricing → checkout flow (≤2 clicks)', async ({ page }) => {
    ...
    const checkoutButton = page.locator('button:has-text("Get Started"), button:has-text("Subscribe")').first();
    ...
    if (await checkoutButton.isVisible()) {
      ...
      const isCheckoutOrAuth = finalUrl.includes('/checkout') ||
  ...
  test('should display locked card upsell with canonical plan names', async ({ page }) => {
    ...
    const isLocked = await intermediateCard.locator('text=/Upgrade/i, text=/Unlock/i').count() > 0;
...
test.describe('Plan Unification - Accessibility', () => {
  ...
  test('should have sufficient color contrast for plan badges', async ({ page }) => {
    ...
    const intermediateBadge = page.getByTestId('pricing-card-intermediate').locator('text=/Most Popular/i');
