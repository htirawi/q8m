...
test.describe('Study Flow', () => {
  ...
  test('should track session time', async ({ page }) => {
    ...
    expect(timerText).toMatch(/0:0[2-9]/); // Should show at least 2 seconds
  });
...
test.describe('Study Flow - Mobile', () => {
  ...
  test('should work on mobile devices', async ({ page }) => {
    ...
    // Timer should be sticky on scroll
    await page.evaluate(() => window.scrollBy(0, 500));
    await expect(page.locator('[data-testid="session-timer"]')).toHaveClass(/sticky/);
  });
  ...
  test('should support swipe gestures', async ({ page }) => {
    ...
    await questionCard.dispatchEvent('touchend', { changedTouches: [{ clientX: 50, clientY: 100 }] });
