...
test.describe("Conversion Modal - Display", () => {
  ...
  test("should display trust badges", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /start free trial|upgrade/i })
      .first()
      .click();
...
test.describe("Conversion Modal - Billing Cycle", () => {
  ...
  test("should toggle between monthly and annual billing", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /annual|yearly/i })).toHaveClass(
      /active|selected/
    );
  ...
  test("should track billing cycle change", async ({ page }) => {
    ...
    // Should track event
    const cycleChangeEvent = analyticsEvents.find(
      (e: unknown) => Array.isArray(e) && e[1] === "convert_plan_changed"
    );
...
test.describe("Conversion Modal - Coupon Code", () => {
  ...
  test("should track coupon application", async ({ page }) => {
    ...
    // Should track event
    const couponEvent = analyticsEvents.find(
      (e: unknown) => Array.isArray(e) && e[1] === "convert_coupon_applied"
    );
...
test.describe("Conversion Modal - Checkout Flow", () => {
  ...
  test("should track checkout initiation", async ({ page }) => {
    ...
    // Should track event
    const checkoutEvent = analyticsEvents.find(
      (e: unknown) => Array.isArray(e) && e[1] === "checkout_opened"
    );
...
test.describe("Conversion Modal - Dismissal", () => {
  ...
  test("should track dismissal with source", async ({ page }) => {
    ...
    const dismissEvent = analyticsEvents.find(
      (e: unknown) => Array.isArray(e) && e[1] === "convert_dismissed"
    );
...
test.describe("Conversion Modal - Accessibility", () => {
  ...
  test("should trap focus within modal", async ({ page }) => {
    ...
    const modalContainsFocus = await page
      .locator('[data-testid="convert-modal"]')
      .evaluate((modal, focused) => modal.contains(focused as Node), focusedElement);
