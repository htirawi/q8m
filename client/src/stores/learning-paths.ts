...
export const useLearningPathsStore = defineStore('learningPaths', () => {
   ...
   */
  const fetchEnrollments = async (status?: 'in-progress' | 'completed' | 'abandoned'): Promise<void> => {
    try {
      loading.value = true;
      error.value = null;
  ...
  const enrollInPath = async (pathId: string): Promise<boolean> => {
    try {
      ...
      const data = await httpClient.post<{
        ...
        `${API_URL}/api/v1/learning-paths/${pathId}/enroll`,
        {},
        { requireAuth: true }
      );
  ...
  const completeModule = async (
    ...
    try {
      ...
      const result = await httpClient.post<{
        ...
        `${API_URL}/api/v1/learning-paths/${pathId}/modules/${moduleId}/complete`,
        data,
        { requireAuth: true }
      );
