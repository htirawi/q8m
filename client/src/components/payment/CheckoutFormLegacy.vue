...
const displayPrice = computed(() => {
  ...
  if (billingCycle.value === "yearly" && props.selectedPlan.planId !== "JUNIOR") {
    ...
    const yearlyPrice = monthlyPrice * 12 * YEARLY_DISCOUNT_MULTIPLIER;
...
const availablePaymentMethods = computed(() => {
  const currency = paymentStore.currentCurrency;
  return getAvailablePaymentMethods(currency).map((method) => ({
    ...method,
    description: t(`checkout.${method.id}Description`),
  }));
});
