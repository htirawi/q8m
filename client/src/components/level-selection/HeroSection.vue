    ...
    <div class="hero-section__content">
      ...
      <div class="hero-section__stats" v-if="showStats && stats">
        ...
        <div class="stat-card stat-card--level" v-if="stats.level !== undefined">
          ...
          <div class="stat-card__value">
            <span>{{ $t('levels.' + currentDifficulty) }}</span>
          </div>
...
const motivationalMessage = computed(() => {
  return t(`motivation.message${currentMessageIndex.value}`, motivationalMessages[currentMessageIndex.value]);
});
...
.hero-section {
  ...
  @apply w-full py-12 px-4 md:px-6 lg:px-8;
...
.hero-section__content {
  ...
  @apply flex flex-col md:flex-row md:items-center md:justify-between gap-8;
}
...
.hero-section__motivation {
  @apply text-sm md:text-base text-gray-700 dark:text-gray-300;
...
.hero-section__quick-stats--mobile {
  @apply flex md:hidden gap-4 mt-4;
}
...
.stat-card__icon {
  @apply w-8 h-8 p-1.5 rounded-lg;
...
.stat-card__badge {
  @apply text-lg;
  @apply hover:scale-125 transition-transform duration-fast;
  @apply cursor-help;
}
