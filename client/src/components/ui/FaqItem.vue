...
<template>
    <div class="faq-item" :data-faq-id="faq.id">
        <button :id="buttonId" type="button" class="faq-item__button" :aria-expanded="internalIsOpen"
            :aria-controls="contentId" @click="toggleOpen" @keydown="handleKeydown">
            <span class="faq-item__question">
                {{ t(faq.qKey) }}
            </span>
            <svg :class="['faq-item__icon', { 'faq-item__icon--open': internalIsOpen }]" fill="none"
                stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
        </button>

        <div v-show="internalIsOpen" :id="contentId" class="faq-item__answer" role="region" :aria-labelledby="buttonId">
