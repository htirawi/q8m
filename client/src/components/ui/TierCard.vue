...
<template>
    <article :class="[
        ...
        <div class="tier-card__price">
            <span class="tier-card__price-amount">{{ t(tier.priceKey) }}
        ...
        <ul class="tier-card__bullets" role="list">
            <li v-for="(bulletKey, idx) in tier.bulletsKey" :key="idx" class="tier-card__bullet">
                <svg class="tier-card__bullet-icon" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
        ...
        <button type="button" class="tier-card__cta" @click="handleCTAClick"
