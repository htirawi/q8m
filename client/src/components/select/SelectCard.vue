<template>
    <article :id="`select-card-${option.id}`" class="select-card" :class="{
        'select-card--selected': isSelected,
        'select-card--locked': option.isLocked,
        'select-card--disabled': option.disabled,
    }" role="button" :tabindex="option.isLocked || option.disabled ? -1 : 0"
        :aria-label="$t('select.cardAriaLabel', { title: $t(option.titleKey) })" :aria-pressed="isSelected"
        :aria-disabled="option.isLocked || option.disabled" @click="handleClick" @keydown.enter.prevent="handleClick"
        ...
        <!-- Badge -->
        <div v-if="option.badge && !option.isLocked" class="select-card__badge"
        ...
        <div class="select-card__content">
            ...
            <div v-if="option.progress && option.progress > 0 && !option.isLocked" class="select-card__progress">
                <div class="progress-bar" role="progressbar" :aria-valuenow="option.progress" aria-valuemin="0"
                    aria-valuemax="100" :aria-label="$t('select.progressLabel', { progress: option.progress })">
                    <div class="progress-fill" :style="{ width: `${option.progress}%` }"></div>
                </div>
                <span class="progress-text">{{ option.progress }}% {{ $t('select.complete') }}

</span>
            </div>

            <!-- CTA -->
            <button v-if="!option.isLocked" type="button" class="select-card__cta" :aria-pressed="isSelected"
                @click.stop="handleClick">
                <span v-if="isSelected">{{ $t('select.selected') }}

</span>
                <span v-else>{{ $t('select.choose') }}

</span>
            </button>

            <button v-else type="button" class="select-card__cta select-card__cta--upgrade" @click.stop="handleUpgrade">
                {{ $t('select.upgrade') }}
...
.select-card:not(.select-card--locked, .select-card--disabled):hover {
    @apply bg-white/15 border-white/30 cursor-pointer;
...
.select-card--selected {
    ...
    box-shadow: 0 20px 60px -10px rgb(0, 0, 0, 0.5), 0 0 0 2px rgb(255, 255, 255, 0.3);
}
...
.select-card__lock-overlay {
    @apply absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm rounded-2xl;
}
...
/* Badge */
.select-card__badge {
    @apply absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider;
    @apply z-20;
}
...
.select-card__subtitle {
    @apply text-sm font-medium text-white/80 uppercase tracking-wide;
}
...
/* CTA */
.select-card__cta {
    @apply mt-4 w-full px-6 py-3 rounded-xl font-bold text-sm;
