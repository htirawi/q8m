    ...
    <div class="select-error" role="alert" aria-live="assertive">
        <div class="select-error__container">
            ...
            <div class="select-error__content">
                <h3 class="select-error__title">{{ $t('errors.somethingWentWrong') }}
            ...
            <div class="select-error__actions">
                <button type="button" class="retry-btn" :disabled="isRetrying" @click="handleRetry">
                    ...
                    <span v-if="!isRetrying">{{ $t('common.retry') }}

</span>
                    <span v-else>{{ $t('common.retrying') }}
...
.select-error__container {
    ...
    @apply bg-red-500/20 backdrop-blur-md rounded-2xl p-8 border border-red-400/30;
    @apply flex flex-col md:flex-row items-center gap-6;

    animation: shake 0.5s ease-out, fade-in 0.3s ease-out;
}
...
.retry-btn {
    @apply px-6 py-3 rounded-xl bg-white text-red-600 font-bold text-sm;
    @apply transition-all duration-200;
    @apply hover:bg-white/90 hover:scale-105;
...
.dismiss-btn {
    @apply px-6 py-3 rounded-xl bg-white/20 text-white font-bold text-sm border border-white/30;
    @apply transition-all duration-200;
