<template>
    <div v-if="hasError" class="error-boundary">
        <div class="error-boundary__container">
            ...
            <h2 class="error-boundary__title">{{ title }}

</h2>
            <p class="error-boundary__message">{{ message }}
            ...
            <div v-if="showDetails && errorDetails" class="error-boundary__details">
                <details>
                    ...
                    <pre
                        class="mt-2 overflow-auto rounded-lg bg-gray-100 p-4 text-xs text-gray-800 dark:bg-gray-800 dark:text-gray-200">{{ errorDetails }}
            ...
            <div class="error-boundary__actions">
                <button v-if="showRetry" @click="handleRetry" class="btn btn--primary" :disabled="isRetrying">
                    <span v-if="isRetrying">{{ $t('common.retrying') }}

...</span>
                    <span v-else>{{ retryText || $t('common.retry') }}
...
.error-boundary__container {
    @apply max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 text-center;
