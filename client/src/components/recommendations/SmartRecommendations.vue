<template>
  <div class="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800">
    ...
    <div v-if="personalizedMessage" class="mb-6 rounded-lg bg-gradient-to-r from-blue-50 to-purple-50 p-4 dark:from-blue-900/20 dark:to-purple-900/20">
      <div class="flex items-start gap-3">
        <svg class="mt-1 h-6 w-6 flex-shrink-0 text-blue-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div v-if="quickTips.length > 0">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        >
          <div class="flex items-start gap-3">
            <svg class="mt-0.5 h-5 w-5 flex-shrink-0 text-yellow-600 dark:text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
...
import type { QuizAttempt, RecommendedQuiz, StudyTopic, LevelStatsMap } from '@/types/quiz-analytics';
...
const generateRecommendations = () => {
  ...
  if (juniorStats && intermediateStats && juniorStats.avgScore >= 80 && intermediateStats.attempts === 0) {
    quizRecs.push({
  ...
  if (intermediateStats && seniorStats && intermediateStats.avgScore >= 80 && seniorStats.attempts === 0) {
    quizRecs.push({
      id: 'next_senior',
      level: 'senior',
