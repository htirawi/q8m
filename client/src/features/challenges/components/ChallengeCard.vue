<template>
  <div
    ...
    <div class="mb-4 flex items-start justify-between">
      <div class="flex items-center gap-3">
        ...
        <div v-if="opponent" :class="[
          ...
          <img v-if="opponent.avatar" :src="opponent.avatar" :alt="opponent.name"
    ...
    <div class="flex gap-2">
      ...
      <button v-if="challenge.status === 'pending' && !isChallenger(challenge)"
        @click="$emit('accept', String(challenge._id || challenge.id || ''))" :disabled="loading"
      ...
      <button v-if="challenge.status === 'pending' && !isChallenger(challenge)"
        @click="$emit('reject', String(challenge._id || challenge.id || ''))" :disabled="loading"
      ...
      <button v-if="challenge.status === 'in-progress' && !hasUserSubmitted(challenge)"
        @click="$emit('start', String(challenge._id || challenge.id || ''))" :disabled="loading"
      ...
      <div v-if="challenge.status === 'in-progress' && hasUserSubmitted(challenge)"
      ...
      <!-- View Details Button -->
      <button @click="$emit('view-details', String(challenge._id || challenge.id || ''))"
