<template>
  <div class="animate-fade-in-up space-y-6">
    <div class="overflow-hidden rounded-3xl border-2 border-gray-200 bg-white/90 shadow-2xl backdrop-blur-sm dark:border-gray-700 dark:bg-gray-800/90">
      <!-- Celebration Header -->
      <div class="relative overflow-hidden bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-8 text-center dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 sm:p-10">
        <!-- Decorative Elements -->
        <div class="pointer-events-none absolute inset-0 overflow-hidden">
          <div class="absolute -left-4 -top-4 h-32 w-32 animate-blob rounded-full bg-blue-300 opacity-20 mix-blend-multiply blur-xl filter dark:bg-blue-600"></div>
          <div class="animation-delay-2000 absolute -right-4 -bottom-4 h-32 w-32 animate-blob rounded-full bg-purple-300 opacity-20 mix-blend-multiply blur-xl filter dark:bg-purple-600"></div>
        </div>
        ...
        <div class="relative">
          ...
          <!-- Title -->
          <h2 class="mb-3 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-4xl font-extrabold text-transparent dark:from-blue-400 dark:via-purple-400 dark:to-pink-400 sm:text-5xl">
            {{ t('quiz.results.title') }}
          </h2>

          <!-- Subtitle with IBadge -->
          <div class="flex items-center justify-center gap-2">
            <span class="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold shadow-md backdrop-blur-sm" :class="levelBadgeClass">
      ...
      <div class="grid gap-4 p-6 sm:grid-cols-3 sm:p-8">
        <!-- Correct Answers -->
        <div class="group relative overflow-hidden rounded-2xl border-2 border-blue-200 bg-blue-50/80 p-6 text-center backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-lg dark:border-blue-800 dark:bg-blue-900/20">
          ...
          <div class="mb-2 text-4xl font-extrabold text-blue-900 dark:text-blue-200">
            {{ score.correct }}<span class="text-2xl text-blue-600 dark:text-blue-400">/{{ score.total }}</span>
          </div>
        ...
        <!-- Accuracy Percentage -->
        <div class="group relative overflow-hidden rounded-2xl border-2 border-green-200 bg-green-50/80 p-6 text-center backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-lg dark:border-green-800 dark:bg-green-900/20">
          <div class="mb-3 flex justify-center">
          ...
          <div class="mb-2 text-4xl font-extrabold text-green-900 dark:text-green-200">
            {{ score.percentage }}<span class="text-2xl">%</span>
          </div>
        ...
        <!-- Time Spent -->
        <div class="group relative overflow-hidden rounded-2xl border-2 border-purple-200 bg-purple-50/80 p-6 text-center backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-lg dark:border-purple-800 dark:bg-purple-900/20">
          <div class="mb-3 flex justify-center">
            <div class="rounded-full bg-purple-100 p-3 dark:bg-purple-900/30">
              <svg class="h-6 w-6 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <div v-if="hasGamificationData && quizResultData" class="border-t border-gray-200 px-6 py-6 dark:border-gray-700 sm:px-8">
        ...
        <div class="space-y-3">
          <!-- XP Earned -->
          <div class="rounded-xl border-2 border-purple-200 bg-purple-50/80 backdrop-blur-sm dark:border-purple-800 dark:bg-purple-900/20">
            <div class="flex items-center justify-between p-4">
              ...
              <div class="flex items-center gap-3">
                <div class="text-2xl font-black text-purple-600 dark:text-purple-400">
                  +<AnimatedCounter :value="quizResultData.xpEarned" :duration="1500" :delay="300" />
                </div>
                <button
                  v-if="hasXPBreakdown"
                  @click="showXPBreakdown = true"
            ...
            <div v-if="hasXPBreakdown && !showXPBreakdown" class="border-t border-purple-200 px-4 py-3 dark:border-purple-800">
              <XPBreakdown
          ...
          <!-- Badges Earned -->
          <div v-if="quizResultData.badgesEarned && quizResultData.badgesEarned.length > 0" class="rounded-xl border-2 border-yellow-200 bg-yellow-50/80 p-4 backdrop-blur-sm dark:border-yellow-800 dark:bg-yellow-900/20">
            ...
            <div class="flex flex-wrap gap-2">
              <span
                v-for="badge in quizResultData.badgesEarned"
                :key="badge"
          ...
          <!-- Streak Maintained -->
          <div class="flex items-center gap-2 rounded-xl border-2 border-orange-200 bg-orange-50/80 p-4 backdrop-blur-sm dark:border-orange-800 dark:bg-orange-900/20">
            <span class="text-2xl">🔥</span>
          ...
          <!-- Strong Categories -->
          <div v-if="quizResultData.strongCategories && quizResultData.strongCategories.length > 0" class="rounded-xl border-2 border-green-200 bg-green-50/80 p-4 backdrop-blur-sm dark:border-green-800 dark:bg-green-900/20">
            <div class="mb-2 flex items-center gap-2">
            ...
            <div class="flex flex-wrap gap-2">
              <span
                v-for="category in quizResultData.strongCategories"
                :key="category"
          ...
          <!-- Weak Categories -->
          <div v-if="quizResultData.weakCategories && quizResultData.weakCategories.length > 0" class="rounded-xl border-2 border-blue-200 bg-blue-50/80 p-4 backdrop-blur-sm dark:border-blue-800 dark:bg-blue-900/20">
            <div class="mb-2 flex items-center gap-2">
              <span class="text-xl">📚</span>
            ...
            <div class="flex flex-wrap gap-2">
              <span
                v-for="category in quizResultData.weakCategories"
                :key="category"
    ...
    <Teleport to="body">
      <div
        v-if="showXPBreakdown && hasXPBreakdown"
