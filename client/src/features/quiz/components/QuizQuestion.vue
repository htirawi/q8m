<template>
  <div class="max-w-4xl mx-auto">
    <!-- Clean Question Card -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 dark:bg-gray-900 dark:border-gray-800">
      <!-- Question Text - Focus Point -->
      <div class="mb-8">
        <h1 class="text-2xl font-semibold leading-relaxed text-gray-900 dark:text-white select-none"
      ...
      <div class="mb-8">
        <!-- Essential Info Only -->
        <div class="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400 select-none">
          ...
          <span class="font-medium">{{ questionTypeBadge }}

</span>
          <span class="text-gray-300 dark:text-gray-600">•</span>
          <span class="text-gray-700 dark:text-gray-300 font-semibold">{{ question.points }}
 ...
 point{{ question.points !==
        ...
        <div v-if="getPrimaryCategory" class="mt-3">
          <span
            class="inline-flex items-center rounded-full bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 select-none">
            {{ getPrimaryCategory }}
      ...
      <div v-if="question.type === 'multiple-choice' || question.type === 'true-false'" class="space-y-3">
        <button v-for="option in options" :key="option.id" type="button" :class="getOptionClass(option.id)"
          ...
          <div class="flex items-center justify-between">
            <span class="text-lg select-none">{{ option.text }}</span>
      ...
      <div v-else-if="question.type === 'fill-blank'" class="space-y-4">
        <input :value="textAnswer" type="text" :disabled="hasAnswered"
          class="w-full rounded-xl border border-gray-200 bg-white px-6 py-4 text-lg text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-colors dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-400"
      ...
      <div v-else-if="question.type === 'multiple-checkbox'" class="space-y-3">
        <label v-for="option in options" :key="option.id" :class="getCheckboxOptionClass(option.id)"
          ...
          <span class="ml-4 text-lg select-none">{{ option.text }}
      ...
      <div v-if="hasAnswered" class="mt-8 rounded-xl bg-blue-50 p-6 dark:bg-blue-900/20 select-none"
        @contextmenu.prevent @selectstart.prevent>
        <h3 class="mb-3 text-lg font-semibold text-blue-900 dark:text-blue-200 select-none">
          {{ t('quiz.explanation') }}

        </h3>
        <div class="text-blue-800 dark:text-blue-300 leading-relaxed select-none" v-html="explanation"></div>
      </div>

      <!-- Action Button -->
      <div class="mt-8 flex justify-center">
        <button v-if="!hasAnswered" type="button"
          class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-8 py-4 text-lg font-medium text-white shadow-sm transition-all hover:bg-blue-700 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
...
const getcheckboxoptionclass = (optionId: string) => {
  const baseClass = 'flex w-full items-center rounded-xl border-2 px-6 py-4 transition-all duration-200';
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  if (isCtrlOrCmd && (
    ...
    event.key === 'v' || event.key === 's' || event.key === 'p' || event.key === 'u'
  )) {
