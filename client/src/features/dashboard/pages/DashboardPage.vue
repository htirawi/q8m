<template>
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <button type="button"
            class="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-lg transition-all hover:shadow-xl hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
            @click="loadDashboardData">
            🔄 Refresh
          </button>
          <button type="button"
            class="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-lg transition-all hover:shadow-xl hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700"
            @click="goToBilling">
            {{ t('dashboard.manage') }}
      ...
      <div v-if="isLoading" class="space-y-8">
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <div v-for="i in 4" :key="i" class="h-32 animate-pulse rounded-xl bg-gray-200 dark:bg-gray-700"></div>
        </div>
      ...
      <div v-else-if="error" class="rounded-xl bg-red-50 p-8 text-center dark:bg-red-900/20">
        <div class="mx-auto mb-4 h-16 w-16 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
        ...
        <h3 class="text-lg font-semibold text-red-900 dark:text-red-100 mb-2">Failed to load dashboard data</h3>
        <p class="text-red-700 dark:text-red-300 mb-4">{{ error }}
      ...
      <div v-else class="space-y-8">
        ...
        <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <!-- Total Quizzes -->
          <div
            class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 p-6 text-white shadow-xl transition-all hover:shadow-2xl hover:scale-105">
            ...
            <div class="relative">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-3xl font-bold">{{ dashboardStats.totalQuizzesCompleted }}

</p>
                  <p class="text-xs text-blue-100">{{ getQuizTrend() }}
          ...
          <!-- Average Score -->
          <div
            class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500 to-green-600 p-6 text-white shadow-xl transition-all hover:shadow-2xl hover:scale-105">
            <div class="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-white/10"></div>
            <div class="relative">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-green-100">Average Score</p>
                  <p class="text-3xl font-bold">{{ getAverageScoreDisplay() }}

</p>
                  <p class="text-xs text-green-100">{{ getScoreTrend() }}
          ...
          <!-- Current Streak -->
          <div
            class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-orange-500 to-orange-600 p-6 text-white shadow-xl transition-all hover:shadow-2xl hover:scale-105">
            <div class="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-white/10"></div>
            <div class="relative">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-orange-100">Current Streak</p>
                  <p class="text-3xl font-bold">{{ dashboardStats.currentStreak }}

</p>
                  <p class="text-xs text-orange-100">{{ getStreakTrend() }}
          ...
          <!-- Total XP -->
          <div
            class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500 to-purple-600 p-6 text-white shadow-xl transition-all hover:shadow-2xl hover:scale-105">
            <div class="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-white/10"></div>
            <div class="relative">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-purple-100">Total XP</p>
                  <p class="text-3xl font-bold">{{ formatNumber(dashboardStats.totalXP) }}

</p>
                  <p class="text-xs text-purple-100">{{ getXPTrend() }}
          ...
          <div class="rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-800">
            ...
            <div class="mb-6">
              <div class="mb-2 flex items-center justify-between">
                <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Current Level</span>
                <span class="text-sm font-bold text-purple-600 dark:text-purple-400">Level {{
                  dashboardStats.currentLevel }}</span>
              </div>
              <div class="h-3 rounded-full bg-gray-200 dark:bg-gray-700">
                <div class="h-3 rounded-full bg-gradient-to-r from-purple-500 to-blue-500"
 ...
 XP to next level
              ...
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-2xl font-bold text-gray-900 dark:text-white">{{
                    formatStudyTime(dashboardStats.totalStudyTime) }}</p>
                </div>
          ...
          <div class="rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-800">
            ...
            <div v-if="dashboardStats.recentActivity.length === 0" class="text-center py-8">
              <div
                class="mx-auto mb-4 h-16 w-16 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
              ...
              <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Complete your first quiz to see activity here</p>
            </div>

            <div v-else class="space-y-4">
              <div v-for="activity in dashboardStats.recentActivity.slice(0, 5)" :key="activity.id"
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-gray-500 dark:text-gray-400">{{ formatTimeAgo(activity.timestamp) }}

</p>
                </div>
                <span class="text-xs font-semibold text-purple-600 dark:text-purple-400">+{{ activity.xp }}
 ...
 XP</span>
        ...
        <!-- Quick Actions -->
        <div class="rounded-2xl bg-gradient-to-r from-purple-500 to-blue-600 p-8 text-white shadow-xl">
          <div class="text-center">
            <h3 class="mb-2 text-2xl font-bold">Ready to continue learning?</h3>
            <p class="mb-6 text-purple-100">Choose your next challenge and keep building your skills</p>
            <div class="flex flex-col gap-4 sm:flex-row sm:justify-center">
              <button @click="goToQuiz"
                class="rounded-xl bg-white px-8 py-3 font-semibold text-purple-600 shadow-lg transition-all hover:shadow-xl hover:scale-105">
                Start New Quiz
              </button>
...
const loadDashboardData = async () => {
  try {
    ...
    const [progressResponse, streakResponse, coinsResponse, quizResultsResponse] = await Promise.allSettled([
    ...
    dashboardStats.value = {
      ...
      averageScore: progressData?.progress?.averageQuizScore || calculateAverageScore(quizData?.results || []),
      currentStreak: progressData?.progress?.streaks?.currentStreak || streakStore.streak.currentStreak || 0,
      totalXP: progressData?.progress?.xp || 0,
      currentLevel: progressData?.progress?.level || Math.max(1, Math.floor((progressData?.progress?.xp || 0) / 100) + 1),
...
watch(() => router.currentRoute.value.path, (newPath, oldPath) => {
  ...
  if (oldPath && (oldPath.includes('/study') || oldPath.includes('/quiz')) &&
    newPath && newPath.includes('/dashboard')) {
    loadDashboardData();
  }
}, { immediate: false });
