<template>
  <div
    class="rounded-xl bg-white p-6 shadow-lg transition-shadow duration-300 hover:shadow-xl dark:bg-gray-800 sm:p-8"
  >
    <div class="mb-6">
      <!-- Question Metadata Header -->
      <div class="mb-4 flex select-none flex-wrap items-center gap-3">
        <!-- Category Badge -->
        <span
          v-if="question.category"
          :class="getCategoryBadgeClass(question.category)"
          class="inline-flex select-none items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold shadow-sm transition-all duration-200 hover:scale-105"
        >
        ...
        <span
          ...
          class="inline-flex select-none items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold shadow-sm"
        >
          <span class="flex gap-0.5">
            <span
              v-for="i in getDifficultyDots(question.difficulty)"
              :key="i"
        ...
        <!-- Question Type Badge -->
        <span
          class="inline-flex select-none items-center gap-1.5 rounded-lg bg-purple-50 px-3 py-1.5 text-xs font-medium text-purple-700 dark:bg-purple-900/20 dark:text-purple-300"
        >
          <svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 20 20">
            <path
              fill-rule="evenodd"
      ...
      <!-- Question Text -->
      <h2
        class="mb-4 select-none text-xl font-bold leading-tight text-gray-900 dark:text-white sm:text-2xl"
        @contextmenu.prevent
        @selectstart.prevent
      >
        {{ questionText }}
    ...
    <div
      v-if="showAnswer && question.type !== 'open-ended'"
      ref="answerSection"
    ...
    <div
      v-else-if="showAnswer && hasInteractiveOptions && question.type !== 'open-ended'"
      class="mb-6"
    >
      <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
      ...
      >
        ...
        >
          <div class="flex items-center">
            <div class="flex-1 select-none text-left">{{ option.text }}</div>
      ...
      <div v-else-if="question.type === 'multiple-checkbox'" class="space-y-3">
        <label v-for="option in options" :key="option.id" :class="getCheckboxClass(option.id)">
          ...
          />
          <span class="ml-3 flex-1 select-none">{{ option.text }} </span>
        </label>
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-3">
      ...
      <template v-else>
        ...
        >
          {{ props.isBookmarked ? "🔖" : "📑" }}
          {{ props.isBookmarked ? t("study.bookmarked") : t("study.bookmark") }}
        </button>
      </template>
    </div>
  </div>
...
const togglecheckbox = (optionId: string) => {
  ...
  newAnswersprops.multipleAnswers.includesprops.multipleAnswers.filter;
...
const getoptionclass = (optionId: string) => {
 ...
 border-primary-500 bg-primary-50 text-primary-900 dark:bg-primary-900/20; dark:text-primary-200`;
  }
...
const getCategoryBadgeClass = (category: string) => {
  ...
  return (
    categoryColors[category] || "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
  );
};
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  if (
    isCtrlOrCmd &&
    (event.key === "c" || // Ctrl+C / Cmd+C
      event.key === "a" || // Ctrl+A / Cmd+A (select all)
      event.key === "x" || // Ctrl+X / Cmd+X
      event.key === "v" || // Ctrl+V / Cmd+V
      event.key === "s" || // Ctrl+S / Cmd+S (save)
      event.key === "p" || // Ctrl+P / Cmd+P (print)
      event.key === "u") // Ctrl+U (view source)
  ) {
