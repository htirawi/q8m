<template>
  <div class="rounded-xl bg-white p-6 shadow-lg transition-shadow duration-300 hover:shadow-xl dark:bg-gray-800 sm:p-8">
    <div class="mb-6">
      ...
      <div class="mb-4 flex flex-wrap items-center gap-3 select-none">
        <!-- Category Badge -->
        <span v-if="question.category" :class="getCategoryBadgeClass(question.category)"
          class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold shadow-sm transition-all duration-200 hover:scale-105 select-none">
        ...
        <span v-if="question.difficulty" :class="getDifficultyBadgeClass(question.difficulty)"
          class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold shadow-sm select-none">
        ...
        <!-- Question Type Badge -->
        <span
          class="inline-flex items-center gap-1.5 rounded-lg bg-purple-50 px-3 py-1.5 text-xs font-medium text-purple-700 dark:bg-purple-900/20 dark:text-purple-300 select-none">
          <svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <!-- Question Text -->
      <h2 class="mb-4 text-xl font-bold leading-tight text-gray-900 dark:text-white sm:text-2xl select-none"
      ...
      <!-- Tags -->
      <div v-if="question.tags && question.tags.length > 0" class="flex flex-wrap gap-2 select-none">
        <span v-for="tag in question.tags" :key="tag"
          class="group inline-flex items-center gap-1 rounded-md bg-gradient-to-r from-gray-50 to-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700 transition-all duration-200 hover:from-gray-100 hover:to-gray-200 hover:shadow-sm dark:from-gray-700 dark:to-gray-600 dark:text-gray-300 dark:hover:from-gray-600 dark:hover:to-gray-500 select-none">
    ...
    <div v-else-if="showAnswer && hasInteractiveOptions && question.type !== 'open-ended'" class="mb-6">
      ...
      <div v-if="question.type === 'multiple-choice' || question.type === 'true-false'" class="space-y-3">
        <button v-for="option in options" :key="option.id" type="button" :class="getOptionClass(option.id)"
          @click="selectOption(option.id)" @contextmenu.prevent @selectstart.prevent>
          <div class="flex items-center">
            <div class="flex-1 text-left select-none">{{ option.text }}</div>
      ...
      <div v-else-if="question.type === 'multiple-checkbox'" class="space-y-3">
        <label v-for="option in options" :key="option.id" :class="getCheckboxClass(option.id)">
          ...
          <span class="ml-3 flex-1 select-none">{{ option.text }}
...
const togglecheckbox = (optionId: string) => {
  const newAnswers = props.multipleAnswers.includes(optionId)
    ...
    : [...props.multipleAnswers, optionId];newAnswersprops.multipleAnswers.includesprops.multipleAnswers.filter
