    ...
    <div class="relative mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-12">
      ...
      <div class="mb-8 animate-fade-in-up">
        <button type="button"
          ...
          aria-label="Go back to mode selection" @click="goBack">
      ...
      <div class="animation-delay-200 mb-12 animate-fade-in-up text-center">
        ...
        <div class="mb-6">
          <h1
            class="mb-3 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-3xl font-extrabold leading-tight text-transparent dark:from-blue-400 dark:via-purple-400 dark:to-pink-400 sm:text-4xl md:text-5xl md:leading-tight lg:text-6xl"
            role="heading" aria-level="1">
            Choose Your Framework
          </h1>
          <p class="mx-auto max-w-2xl px-2 text-sm leading-relaxed text-gray-600 dark:text-gray-300 sm:px-0 sm:text-base md:text-lg"
        ...
        <div class="flex flex-wrap items-center justify-center gap-2 px-2 sm:gap-3 sm:px-0">
          <div
            class="flex items-center gap-1.5 rounded-full border border-green-200 bg-green-50/80 px-3 py-1.5 text-xs font-medium text-green-700 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-md dark:border-green-800 dark:bg-green-900/20 dark:text-green-300 sm:gap-2 sm:px-4 sm:py-2 sm:text-sm">
            <svg class="h-3.5 w-3.5 flex-shrink-0 sm:h-4 sm:w-4" fill="currentColor" viewBox="0 0 20 20"
              aria-hidden="true">
          ...
          <div
            class="flex items-center gap-1.5 rounded-full border border-blue-200 bg-blue-50/80 px-3 py-1.5 text-xs font-medium text-blue-700 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-md dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300 sm:gap-2 sm:px-4 sm:py-2 sm:text-sm">
            <svg class="h-3.5 w-3.5 flex-shrink-0 sm:h-4 sm:w-4" fill="currentColor" viewBox="0 0 20 20"
              aria-hidden="true">
              <path
          ...
          <div
            class="flex items-center gap-1.5 rounded-full border border-orange-200 bg-orange-50/80 px-3 py-1.5 text-xs font-medium text-orange-700 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-md dark:border-orange-800 dark:bg-orange-900/20 dark:text-orange-300 sm:gap-2 sm:px-4 sm:py-2 sm:text-sm">
            <svg class="h-3.5 w-3.5 flex-shrink-0 sm:h-4 sm:w-4" fill="currentColor" viewBox="0 0 20 20"
              aria-hidden="true">
              <path fill-rule="evenodd"
      ...
      <div v-if="isLoading" class="animation-delay-400 animate-fade-in-up">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <div v-for="i in 5" :key="i" class="animate-pulse">
            <div
              class="flex h-full flex-col rounded-2xl border-2 border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
              <!-- Icon Skeleton -->
              <div class="mb-4 flex justify-center">
                <div
                  class="h-20 w-20 rounded-2xl bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600">
                </div>
              </div>
              ...
              <div class="mt-auto flex justify-center">
                <div
                  class="h-12 w-40 rounded-xl bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600">
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-else class="animation-delay-400 animate-fade-in-up">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5" role="region"
          aria-label="Framework selection cards">
          <!-- Dynamic Framework Cards from Database -->
          <FrameworkCard v-for="rule in frameworkAccessRules" :key="rule.framework" :framework="rule.framework"
            :question-count="frameworkCounts[rule.framework as keyof typeof frameworkCounts] || 0"
            :icon="rule.metadata.icon" :title="rule.displayName" :description="rule.description"
            :color="rule.metadata.color as 'red' | 'black' | 'blue' | 'purple' | 'gray' | 'gradient'"
            :difficulty="difficulty" :is-locked="rule.isLocked" :required-plan="rule.requiredPlanTier"
      ...
      <div class="animation-delay-600 mt-8 animate-fade-in-up text-center sm:mt-12">
        <div
          class="mx-auto max-w-2xl rounded-2xl border border-gray-200 bg-white/60 p-4 backdrop-blur-sm dark:border-gray-700 dark:bg-gray-800/60 sm:p-6">
          <div class="mb-2 flex justify-center sm:mb-3">
            <div class="rounded-full bg-blue-100 p-2.5 dark:bg-blue-900/30 sm:p-3">
              <svg class="h-5 w-5 text-blue-600 dark:text-blue-400 sm:h-6 sm:w-6" fill="none" stroke="currentColor"
          ...
          <p class="text-xs text-gray-600 dark:text-gray-400 sm:text-sm">
            Select <span class="font-semibold text-purple-600 dark:text-purple-400">Random Mix</span> to practice
            questions from all frameworks. You can switch anytime!
          </p>
        </div>
      </div>
    </div>
