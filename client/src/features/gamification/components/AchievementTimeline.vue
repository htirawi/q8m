<template>
  <div class="achievement-timeline">
    ...
    <div v-if="showHeader" class="mb-6">
      <h2 class="mb-2 text-2xl font-bold text-gray-900 dark:text-white">IAchievement History</h2>
      <p class="text-sm text-gray-600 dark:text-gray-400">Your journey of learning and progress</p>
    </div>
    ...
    <div v-else-if="filteredAchievements.length === 0" class="py-12 text-center">
      ...
      <h3 class="mb-2 text-lg font-semibold text-gray-900 dark:text-white">No achievements yet</h3>
      <p class="text-gray-600 dark:text-gray-400">Start learning to earn badges and level up!</p>
    </div>
    ...
    <div v-else class="relative space-y-6">
      <!-- Timeline Line -->
      <div
        class="absolute bottom-0 left-6 top-0 w-0.5 bg-gradient-to-b from-purple-500 via-indigo-500 to-purple-500 dark:from-purple-600 dark:via-indigo-600 dark:to-purple-600"
      ></div>
      ...
      <TransitionGroup name="timeline-item">
        ...
        >
          ...
          >
            ...
            <!-- Hover Indicator -->
            <div
              class="h-1 bg-gradient-to-r from-transparent via-purple-500 to-transparent opacity-0 transition-opacity group-hover:opacity-100"
            ></div>
          </div>
        </div>
      </TransitionGroup>
...
function getCardClass(type: string): string {
  const classes = {
    badge:
      "border-yellow-200 bg-yellow-50/50 hover:border-yellow-300 dark:border-yellow-800 dark:bg-yellow-900/10 dark:hover:border-yellow-700",
    level:
      "border-purple-200 bg-purple-50/50 hover:border-purple-300 dark:border-purple-800 dark:bg-purple-900/10 dark:hover:border-purple-700",
    milestone:
      "border-pink-200 bg-pink-50/50 hover:border-pink-300 dark:border-pink-800 dark:bg-pink-900/10 dark:hover:border-pink-700",
    streak:
      "border-orange-200 bg-orange-50/50 hover:border-orange-300 dark:border-orange-800 dark:bg-orange-900/10 dark:hover:border-orange-700",
  };
  return classes[type as keyof typeof classes] || classes.badge;
}
