        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <div class="flex items-start gap-3 rounded-lg bg-blue-100 p-4 dark:bg-blue-900/30">
            ...
            <div>
              ...
              <div class="text-sm text-blue-700 dark:text-blue-300">Answer correctly on first attempt</div>
            </div>
          </div>
          <div class="flex items-start gap-3 rounded-lg bg-blue-100 p-4 dark:bg-blue-900/30">
            <span class="text-2xl">⚡</span>
          ...
          <div class="flex items-start gap-3 rounded-lg bg-blue-100 p-4 dark:bg-blue-900/30">
            ...
            <div>
              ...
              <div class="text-sm text-blue-700 dark:text-blue-300">Maintain daily learning streak</div>
            </div>
          </div>
          <div class="flex items-start gap-3 rounded-lg bg-blue-100 p-4 dark:bg-blue-900/30">
            <span class="text-2xl">💯</span>
            <div>
              ...
              <div class="text-sm text-blue-700 dark:text-blue-300">Get 100% accuracy for massive bonus</div>
            </div>
          </div>
        </div>
      </div>
...
import type { IXPSource, IMultiplier, IXPBreakdownProps as Props } from "@/types/components/gamification";
...
const breakdownItems = computed<(IXPSource & { percentage?: number })[]>(() => {
  return Object.entries(props.breakdown).map(([key, xp]) => {
    const sourceInfo = sourceLabels[key] || { label: key, icon: '⭐' };
    const percentage = props.variant === 'detailed' ? Math.round((xp / totalXP.value) * 100) : undefined;
